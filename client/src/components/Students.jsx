...
export default function Students() {
  ...
  const handleSaveStudent = async (studentData) => {
    try {
      if (editStudent) {
        const res = await axios.put(`/students/${editStudent._id}`, studentData);
        setStudents((prev) =>
  ...
  if (error)
    return (
      <div className="flex justify-center items-center h-64">
        <p className="text-red-600 bg-red-50 px-6 py-3 rounded shadow">{error}</p>
      </div>
    );
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-extrabold text-blue-800 tracking-tight">
          <span className="inline-block align-middle mr-2">
            <svg className="w-8 h-8 text-blue-500 inline" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"> 
              <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m13-6.13A4 4 0 0012 4a4 4 0 00-4 4v1a4 4 0 004 4 4 4 0 004-4V8z" />
            </svg>
          </span>
          Students
        </h1>
          ...
          <tbody>
            {students.length === 0 ? (
              ...
              students.map((student, idx) => (
                <tr
                  key={student._id}
                  className={`border-t ${idx % 2 === 0 ? "bg-gray-50" : "bg-white"} hover:bg-blue-50 transition`}
                >
                  ...
                  <td className="py-3 px-4 space-x-2">
                    ...
                    >
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536M9 13l6-6m2 2l-6 6m2 2H7v-4a2 2 0 012-2h4" />
                      </svg>
                      Edit
                    </button>
                    <button
